    ...
    public class QcCylinderMesh : QcPrimitivesBase
    {
        ...
        public class QcCylinderProperties : QcBaseProperties
        {
            ...
            public void CopyFrom(QcCylinderProperties source)
            {
                ...
                this.faceNormals = source.faceNormals;
            }
        ...
        private void GenerateCylinderVertices(float height, int numCircleSegments, float[] xc, float[] yc, bool sideCap)
        {
            ...
            for (int i = 0; i <= numCircleSegments; i++)
            {
                ...
                {
                    Vector3 normal1 = new Vector3((x[numCircleSegments] * x[0]) * 0.5f, 0f, (y[numCircleSegments] + y[0]) * 0.5f);
                    normal1.Normalize();
                    AddNormal(normal1);
                    AddNormal(normal1);
                }
                ...
                for (int i = 0; i <= numCircleSegments; i++)
                {
                    ...
                    else
                    {
                        Vector3 normal1 = new Vector3(
                            (xc[numCircleSegments] * xc[0]) * 0.5f * Mathf.Cos(coneAngle), 
                            Mathf.Sin(coneAngle), 
                            (yc[numCircleSegments] + yc[0]) * 0.5f * Mathf.Cos(coneAngle));
                        normal1.Normalize();
                        AddNormal(normal1);
                        AddNormal(normal1);
                    }
                                                     ...
                                                     float[] xc, float[] yc, bool sideCap)
        {
            ...
            for (int i = 0; i <= numCircleSegments; i++)
            {
                ...
                if (!properties.faceNormals)
                {
                    Vector3 normal1 = new Vector3(xc[i], 0f, yc[i]);
                    normal1.Normalize();
                    AddNormal(normal1);
                    AddNormal(normal1);

                    Vector3 normalBottomBevel = new Vector3(xc[i], -properties.radius, yc[i]);
                    normalBottomBevel.Normalize();
                    AddNormal(normalBottomBevel);
                    AddNormal(normalBottomBevel);

                    Vector3 normalTopBevel = new Vector3(xc[i], properties.radius, yc[i]);
                ...
                {
                    Vector3 normal1 = new Vector3((xc[numCircleSegments] * x[0]) * 0.5f, 0f, (yc[numCircleSegments] + yc[0]) * 0.5f);
                    normal1.Normalize();
                    AddNormal(normal1);
                    AddNormal(normal1);

                    Vector3 normalBottomBevel = new Vector3((xc[numCircleSegments] * x[0]) * 0.5f, -properties.radius, (yc[numCircleSegments] + yc[0]) * 0.5f);
                    normalBottomBevel.Normalize();
                    AddNormal(normalBottomBevel);
                    AddNormal(normalBottomBevel);

                    Vector3 normalTopBevel = new Vector3((xc[numCircleSegments] * x[0]) * 0.5f, properties.radius, (yc[numCircleSegments] + yc[0]) * 0.5f);
                    normalTopBevel.Normalize();
                    AddNormal(normalTopBevel);
                    AddNormal(normalTopBevel);
                }
            ...
            if (height == properties.hollow.height)
            {
                for (int i = 0; i <= numCircleSegments; i++)
                {
                    ...
                    else
                    {
                        ...
                        Vector3 normal2 = new Vector3(-(x[numCircleSegments] * x[0]) * 0.5f, 0f, -(y[numCircleSegments] + y[0]) * 0.5f);
                        normal2.Normalize();
                        AddNormal(normal2);
                        AddNormal(normal2);
                    }
